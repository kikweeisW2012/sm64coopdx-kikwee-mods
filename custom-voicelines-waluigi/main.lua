-- name: \\#f200fa\\Custom \\#6a0dad\\waluigi \\#00faf6\\voicelines
...
local function use_custom_voice(m)
    return m.character.type == CT_WALUIGI -- Only true for waluigi
end
...
function play_custom_character_sound(m, voice)
    if not use_custom_voice(m) then
        return nil  -- Early return if not waluigi
    end
...
local function custom_character_sound(m, characterSound)
    if not use_custom_voice(m) then
        return nil -- Return nil to allow default sound if not waluigi
    end
