-- name: \\#f200fa\\Custom \\#F4260F\\Toad \\#00faf6\\voicelines
...
local function use_custom_voice(m)
    return m.character.type == CT_TOAD -- Only true for Toad
end
...
function play_custom_character_sound(m, voice)
    if not use_custom_voice(m) then
        return nil  -- Early return if not toad
    end
...
local function custom_character_sound(m, characterSound)
    if not use_custom_voice(m) then
        return nil -- Return nil to allow default sound if not toad
    end
