-- name: \\#f200fa\\Custom \\#00FF00\\Luigi \\#00faf6\\voicelines
...
local function use_custom_voice(m)
    return m.character.type == CT_LUIGI -- Only true for Luigi
end
...
function play_custom_character_sound(m, voice)
    if not use_custom_voice(m) then
        return nil  -- Early return if not Luigi
    end
...
local function custom_character_sound(m, characterSound)
    if not use_custom_voice(m) then
        return nil -- Return nil to allow default sound if not Luigi
    end
